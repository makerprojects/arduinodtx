...
void ArduinotxBuzz::Refresh(byte reset_byt) {
	...
	static unsigned long Period_ulng = 0;
	...
	if (reset_byt) {
		...
		unsigned long now_ulng = micros();
		if (now_ulng >= Begin_ulng + (10000L * Pulse_byt) || now_ulng < Begin_ulng) {
			++Pulse_idx_byt;
		...
		if (Pulse_byt) {
			if (Buzz_state_byt) {
				if (now_ulng > Period_ulng + HalfPeriod_int || now_ulng < Period_ulng ) {
					// toggle sound wave
					Wave_state_byt = !Wave_state_byt;
					Period_ulng = now_ulng;
